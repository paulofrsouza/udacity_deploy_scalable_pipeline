...
def split_data_(get_data):
    ...
    try:
        ...
        logging.info(
            'SUCCESS - split_data: train and test datasets were created')
...
def get_metrics_(process_data, get_preds):
    ...
    try:
        ...
        logging.error(
            ' - get_metrics: model performance coud not be calculated')
        raise err
...
def test_train_model(train_model):
    ...
    try:
        assert isinstance(model, type(rf))
    except AssertionError as err:
        logging.error(
            ' - test_train_model: the model was not correctly trained')
        raise err

    logging.info('SUCCESS - test_train_model: the model was correctly trained')


def test_inference(get_preds):
    '''
    ...
    try:
        ...
        logging.error(
            ' - test_inference: the model did not generate predictions')
        raise err
...
def test_compute_model_metrics(get_metrics):
    ...
    try:
        ...
        logging.error(
            ' - test_model_metrics: performance metrics were not calculated')
        raise err

    logging.info(
        'SUCCESS - test_model_metrics: performance metrics calculated')
